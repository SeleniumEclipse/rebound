...
- Block hits use the short **UI Pop Up** sound by Marevnik, licensed CC BY 4.0, with silence trimmed and a softer level. Close hits are mixed without cutting off earlier pops. In **Settings**, turn Sound on and use **Test sound**; the phone's volume buttons adjust media volume. The app never raises system volume automatically.
...
- Short radius-aware aiming hints, ceiling flush with the top row and walls flush with the outer block columns, capped visual effects, sequential volleys up to 999 balls, and deterministic random rows.
- Earlier saves are migrated to the tighter walls and ceiling without clearing progress. A ball stranded in a removed lane is safely returned if moving it inward would overlap a block.
